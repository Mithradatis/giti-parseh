  ...
  useMemo(() => {
    const getSlides = async () => {
      ...
      await gallery.data.forEach((slide: any) => {
        slides.push(
          process.env.API_URL + slide.attributes.Image.data.attributes.url
        )
      });
  ...
  return (
    images.length > 0 && (
      <>
        ...
        <div className={"py-4 md:px-8 px-6 relative bg-white border border-gray-200 w-4/5 m-auto rounded-2xl left-0 right-0 shadow-md"} id="advanced-search" style={{ marginTop: '-4rem' }}>
          <h3 className={"font-bold text-2xl mb-4 text-center"}>
            {
              trans.advanced_product_search
            }
          </h3>
          <div className={"pb-4 flex md:flex-row flex-column flex-wrap items-center justify-center gap-6"}>
            <div className={"md:flex-1 flex-auto"}>
              <select id="countries" className="font-bold h-12 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
