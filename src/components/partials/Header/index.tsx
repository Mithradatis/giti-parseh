    ...
    <header className={'p-5'}>
      ...
      >
        <div className={'flex items-center justify-center gap-4 md:mb-0 mb-6'} id="social-box">
          <Link href={currentPath.replace(`/${locale}`, `/en`)}>EN</Link>
          <Link href={currentPath.replace(`/${locale}`, `/fa`)}>FA</Link>
          <Social />
        </div>
        <h1 className={'flex items-center justify-center md:mb-0 mb-6'}>
          <Image width={75} src={Logo} alt="Logo" className={'mx-2'} />
          <span className={'text-3xl capitalize'}>{trans.website_title}</span>
        </h1>
      ...
      <div className={'container m-auto flex flex-wrap items-center justify-between pt-4'}>
        ...
        >
          ...
          <Link href="#" className={'text-xl font-bold'}>
            <FontAwesomeIcon icon={faShoppingCart} style={{ width: '25px' }} />
          </Link>
        </div>
      </div>
    </header>
